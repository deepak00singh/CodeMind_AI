...
export default function OptimizationCard({ opt, index, codeLine, onReveal }) {
  const [copied, setCopied] = useState(false);

  function handleCopy() {
    if (!opt.example) return;
    navigator.clipboard.writeText(opt.example).then(() => {
      setCopied(true);
  ...
  function handleReveal() {
    if (onReveal && opt.line_number) onReveal(opt.line_number, "warning");
  }
    ...
    >
      ...
      <div style={{
        ...
        {onReveal && opt.line_number && (
          ...
          >
            <span style={{ fontSize: 10 }}>↗</span> Jump to Line
          </motion.button>
        )}
      </div>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "stretch" }}>
            <div style={{
              ...
              <span className="mono" style={{
                fontSize: 11, color: "#f59e0b", fontWeight: 700, userSelect: "none",
              }}>
                {opt.line_number}
              </span>
            </div>
      ...
      {opt.example && (
        <div style={{
          ...
          <div style={{
            ...
            <span className="mono" style={{
              fontSize: 10, fontWeight: 800, letterSpacing: "0.08em", color: "#06b6d4",
            }}>
              IMPROVED CODE
            </span>
            <motion.button
